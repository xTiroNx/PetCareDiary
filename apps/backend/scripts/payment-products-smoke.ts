...
process.env.MONTHLY_PRICE_STARS = "99";
process.env.SIX_MONTHS_PRICE_STARS = "499";
...
await assertInvoice("MONTHLY", 99);
await assertInvoice("SIX_MONTHS", 499);
await expectHttpError("PAYMENT_PRODUCT_UNAVAILABLE", () => createStarsInvoice("user-products-smoke", "YEARLY" as ProductType));
...
await assertGrantDuration("MONTHLY", 99, 30);
await assertGrantDuration("SIX_MONTHS", 499, 180);
await assertGrantDuration("YEARLY", 899, 365);
